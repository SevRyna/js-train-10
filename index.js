...
function customEntries(obj) {
  ...
  let prop = Array();
  prop = Object.entries(obj).map(([key, val]) => [key, val]);
...
function filterObjectsByKey(arr, key) {
  ...
  function getBExists(item) {
    let ii = Array();
    ii.push(item);
    let res = item.hasOwnProperty("b");
    if (res) {
      return item;
    } else {
      return false;
  ...
  let array = arr.filter(getBExists);
